        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div class="flex items-center gap-6 border-l-4 border-yellow-500 pl-6">
            ...
            <div>
              ...
              <div class="text-4xl font-black text-yellow-300">{{ energyToday }} <span class="text-xl text-gray-500">kWh</span></div>
            </div>
          </div>
          <div class="flex items-center gap-6 border-l-4 border-amber-500 pl-6">
            ...
            <div>
              ...
              <div class="text-4xl font-black text-amber-300">{{ batteryCycles }}</div>
            </div>
          </div>
          <div class="flex items-center gap-6 border-l-4 border-green-500 pl-6">
            ...
            <div>
              ...
              <div class="text-4xl font-black text-green-300">{{ uptime }}<span class="text-xl text-gray-500">%</span></div>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  async mounted() {
    try {
      const res = await fetch(`${config.apiBaseUrl}/api/dashboard/`)
      const data = await res.json()
      this.powerSource   = data.power.source
      this.batteryLevel  = data.battery.level
      this.currentPrice  = data.power.currentPrice.toFixed(2)
      this.todaySavings  = data.savings.today.toFixed(2)
      this.energyToday   = data.energy.usedToday
      this.batteryCycles = data.system.batteryCycles
